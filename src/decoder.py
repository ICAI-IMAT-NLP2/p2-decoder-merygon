...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model: int, num_attention_heads: int):
        ...
        for _ in range(num_attention_heads):
            self.heads.append(AttentionHead(d_model, d_model // num_attention_heads, d_model // num_attention_heads, d_model // num_attention_heads))
    ...
    def forward(self, hidden_state, mask=None):
        ...
        concatenated = torch.cat(head_outputs, dim=-1)  # (batch_size, seq_len, d_model * num_attention_heads)
        x = self.output_linear(concatenated)  # (batch_size, seq_len, d_model)
        return x
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        x = self.layer_norm_1(x + self.self_attention(self.layer_norm_1(x), mask))
        x = self.layer_norm_2(x + self.feed_forward(self.layer_norm_2(x)))
...
class Embeddings(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        embeddings = self.token_embeddings(input_ids) + self.position_embeddings(torch.arange(input_ids.size(1), device=input_ids.device).unsqueeze(0).expand(input_ids.size(0), -1))
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        mask = torch.tril(torch.ones((seq_len, seq_len), device=input_ids.device)).unsqueeze(0).expand(input_ids.size(0), -1, -1)  # (batch_size, seq_len, seq_len)
